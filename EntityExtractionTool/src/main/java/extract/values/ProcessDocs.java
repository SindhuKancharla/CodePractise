...
public class ProcessDocs extends GetAllImpValues{
	...
	static ArrayList<String> entities = new ArrayList<String>();
...
//		}
		...
		while(it.hasNext()){
			...
			for(Object i : ids){
				...
				if(k.getType().equalsIgnoreCase("CONTENT")){
					content = k.getFeatures().get("string").toString();
				}
				...
				/*if(k.getType().equalsIgnoreCase("Concepts")){
					if(k.getFeatures().containsKey("string"))
					{
	...
	//	bwimps.close();
		...
		ApacheStanbolEntities.getStanbolEntities(fileType,content);
		RemoveDupsFromTxt.deduplicateTextFile(stanbolOPFile);
...
//		}
		 if(type.equalsIgnoreCase("Annie")){
				tag = k.getFeatures().get("tag").toString();
