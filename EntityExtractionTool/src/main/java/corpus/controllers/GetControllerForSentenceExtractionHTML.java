...
public class GetControllerForSentenceExtractionHTML extends SentenceExtraction {
	...
	public static CorpusController createController() throws GateException, IOException {
		...
		paramsg.put("listsURL", new File(resourcesPath+"resources/sentence.extraction.html/abr.def").toURI().toURL()); 
		
    	LanguageAnalyser mainGazetteer = (LanguageAnalyser)Factory.createResource(  
	     "gate.creole.gazetteer.DefaultGazetteer", Factory.newFeatureMap());
    	 
    	mainGazetteer.setParameterValues(paramsg);
		...
		params.put("grammarURL", new File(resourcesPath+"resources/sentence.extraction.html/MainSentExtraction.jape").toURI().toURL());
